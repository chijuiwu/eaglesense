...
public:
	...
	virtual bool Image<T>::imread_opencv_gray(const cv::Mat& im);
	virtual void Image<T>::convert_to_opencv_mat(cv::Mat& data) const;
//#ifndef _MATLAB
...
template <class T>
bool Image<T>::imread_opencv_gray(const cv::Mat& im)
{
	clear();
	if(ImageIO::loadImage(im,pData,imWidth,imHeight,nChannels))
	{
		computeDimension();
		colorType = GRAY; // when we use qt or opencv to load the image, it's often BGR
		return true;
	}
	return false;
}
...
template <class T>
void Image<T>::convert_to_opencv_mat(cv::Mat& data) const
{
	int offset = 0;
	for (int i = 0; i<imHeight; i++)
		for (int j = 0; j<imWidth; j++)
			for (int k = 0; k<nChannels; k++)
				data.at<cv::Vec2i>(i, j) = pData[offset++];
}
