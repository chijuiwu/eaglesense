...
public:
	template <class T>
	static bool loadImage(const cv::Mat& im, T*& pImagePlane, int& width, int& height, int& nchannels);
	template <class T>
...
template <class T>
bool ImageIO::loadImage(const cv::Mat& im, T *&pImagePlane, int &width, int &height, int &nchannels)
{
